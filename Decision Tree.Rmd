...
active <- df[df$status_code == 'active',]
active %>% View

train <- df[df$status_code != 'active',]
train %>% View
...
```
```{r}
#Testing the Decision Tree on the Test Dataset
test <- df[130:nrow(df),]
...
```{r}
#Testing the Decision Tree on the Active Clients
pred2 <- predict(model, active) 
pred2
```
